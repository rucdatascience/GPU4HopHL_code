...
#include <build_in_progress/HL/HL4GST/hop_constrained_two_hop_labels.h>
...
void _2023WWW_thread_function(int v_k) {
	...
	for (int h = 0; h <= global_upper_k; h++) {
		...
		for (auto u : Vh[h]) {
			...
			//if (u < v_k) { // rank pruning 貌似并不能明显加速_2023WWW_thread_function，_2023WWW_thread_function之所以慢是因为BFS生成了过多的冗余的需要被清洗的label（冗余的比非冗余的多几乎一个数量级）
			//	continue;
			//}
