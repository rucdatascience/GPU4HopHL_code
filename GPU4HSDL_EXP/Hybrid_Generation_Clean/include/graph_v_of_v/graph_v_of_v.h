...
template <typename weight_type> class ARRAY_graph;

template <typename weight_type> // weight_type may be int, long long int, float,
                                // double...
                                class graph_v_of_v {
public:
  /*
  ...
  */
  ...
  inline void add_edge(int, int,
                       weight_type); // this function can change edge weights
  ...
  inline int
  search_adjv_by_weight(int,
                        weight_type); // return first e2 such that w(e1,e2) =
...
template <typename weight_type> void graph_v_of_v<weight_type>::print() {
...
template <typename weight_type> void graph_v_of_v<weight_type>::clear() {

  return std::vector<std::vector<std::pair<int, weight_type>>>().swap(ADJs);
}

template <typename weight_type> int graph_v_of_v<weight_type>::degree(int v) {
...
void graph_v_of_v<weight_type>::txt_save(std::string save_name) {
  ...
  outputFile << "|E|= " << graph_v_of_v<weight_type>::edge_number()
             << std::endl;
  outputFile << std::endl;
    ...
    // 解析表头，获取顶点数，ADJs.resize(n)
    {
      ...
      if (pos != std::string::npos) {
        ...
        if (iss >> V) {
          ...
          std::cerr << "无法解析 |V| 的值。" << std::endl;
          // 处理错误，例如退出或设置默认值
          exit(1);
        }
      } else {
        std::cerr << "表头中未找到 '|V| ='" << std::endl;
        // 处理错误，例如退出或设置默认值
        exit(1);
      }
    }
...
// parse_string(line_content, " ");

// 			if (!Parsed_content[0].compare("|V|=")) // when it's equal,
// compare returns 0
// 			{
...
// 		std::cout << "Unable to open file " << save_name << std::endl
// 			<< "Please check the file location or file name." <<
// std::endl; // throw an error message 		getchar(); // keep the console window
// 		exit(1); // end the program
// 	}
// }
...
public:
  ...
  */
  ...
  std::vector<weight_type>
      Edge_weights; // Edge_weights[Neighbor_start_pointers[i]] is the start of
                    // Neighbor_sizes[i] edge weights
};
...
static void graph_v_of_v_example() {
